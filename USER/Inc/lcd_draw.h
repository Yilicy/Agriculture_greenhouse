...
#define GRED 		0XFFE0
#define GBLUE		0X07FF
#define RED         0X7800 // 0xF800
#define MAGENTA     0xF81F
#define GREEN       0X03E0 // 0x07E0
#define CYAN        0x7FFF
...
#define DARK_GREEN  0x01E0  // #001F00
#define DIM_GREEN   0x0040  // #000A00
#define MID_GREEN   0x00A0  // #010b01
...
void lcd_draw_rectangle(uint16_t x,uint16_t y,uint16_t wide,uint16_t height,uint16_t color);  // 画矩形
...
void lcd_show_string(uint16_t x, uint16_t y, uint8_t size, char *p, uint16_t color,uint16_t back_color);  /* 显示字符串函数 */
...
void lcd_shownum(uint16_t x,uint16_t y,uint8_t sizey,uint16_t num,uint16_t color);  /* 显示数字 */
void lcd_showpicture(uint16_t x,uint16_t y,uint16_t width,uint16_t length, const uint8_t pic[]); /* 显示图片 */
